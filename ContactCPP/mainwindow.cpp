...
void MainWindow::DatabaseInit()
{
    ...
    if (!query.isActive()){
        ...
        DatabasePopulate();
        qWarning() << "db didn't exist, we have populate the db";
    }
    populateListWithDB();
}
...
void MainWindow::DatabasePopulate()
{
    ...
    if (!query.exec("INSERT INTO contact(lastname, firstname, email, numberphone, commentary) VALUES('Matas', 'Lucas', 'luma@gmail.com', '0601010101', 'baaaaaa')"))
        qWarning() << "MainWindow::DatabasePopulate - ERROR: " << query.lastError().text();
    if (!query.exec("INSERT INTO contact(lastname, firstname, email, numberphone, commentary) VALUES('Lalande', 'Jason', 'laja@gmail.com', '0655555555', 'buuuuuuuu')"))
        qWarning() << "MainWindow::DatabasePopulate - ERROR: " << query.lastError().text();
    if (!query.exec("INSERT INTO contact(lastname, firstname, email, numberphone, commentary) VALUES('Modolo', 'Thomas', 'moto@gmail.com', '0654545454', 'beeeeeeeeee')"))
        qWarning() << "MainWindow::DatabasePopulate - ERROR: " << query.lastError().text();
...
void MainWindow::populateListWithDB()
{
    ...
    if( !query.exec() )
    {
        qWarning() << "MainWindow::DatabasePopulateList - ERROR: tchip " << query.lastError().text();
        return;
    }
...
void MainWindow::saveFileContacts()
{
    ...
    if (contactFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        QString error = "Impossible de sauvegarder le fichier " + m_contactsFileName;
        qCritical() << error;
        ui->statusbar->showMessage(error);
    }
}
